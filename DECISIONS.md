...
- **Consequences**: Releasing is a version bump in two files, a commit, and a `v<version>` tag — there is no manual publish step and no way to publish a version the tag disagrees with. Until someone creates `NUGET_API_KEY` the release job fails at the push step and nothing is published; CI is unaffected, since it packs but never pushes. nuget.org versions are immutable, so a bad release is rolled back by unlisting it and shipping a patch, not by deleting the tag. The README gains an Installation section naming the package id and the registry, and `.project/INFRASTRUCTURE.md` describes the workflow step by step.
