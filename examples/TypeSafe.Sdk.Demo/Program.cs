...
Console.WriteLine("Available models: " + string.Join(", ", models.Models.Select(m => m.Name)));
...
{
    Console.Error.WriteLine($"API error {error.Status} (request {error.RequestId ?? "unknown"}): {error.Body?.ToJsonString() ?? "(no body)"}");
    return 1;
}
